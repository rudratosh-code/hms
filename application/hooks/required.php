...
function required() {
    ...
    // Language
    // Currency
    if ($RTR->class != "cronjobs" && $RTR->class != "payu" && $RTR->class != "status" &&   $RTR->class != "auth" && $RTR->class != "frontend") {
        if (!$CI->ion_auth->in_group(array('superadmin'))) {
            $CI->db->where('hospital_id', $CI->hospital_id);
    ...
    if ($RTR->class != "cronjobs" && $RTR->class != "payu" && $RTR->class != "status"  && $CI->ion_auth->in_group(array('admin', 'superadmin')) && $RTR->class != "auth") {
    ...
    if ( $RTR->class != "cronjobs" && $RTR->class != "payu" && $RTR->class != "status"  && $RTR->class != "frontend" && $RTR->class != "request" && $RTR->class != "auth" && $RTR->class != "site" && $RTR->class != "api") {
        if (!$CI->ion_auth->in_group(array('superadmin'))) {
            if ($CI->ion_auth->in_group(array('admin'))) {
                $current_user_id = $CI->ion_auth->user()->row()->id;
    ...
    $common = array('payu', 'status','macro','auth','pservice', 'frontend', 'settings','import', 'home', 'profile', 'request', 'site', 'api','cronjobs','logs');
