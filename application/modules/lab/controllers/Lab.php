...
class Lab extends MX_Controller
{
    ...
    function getTestStatusLab()
    {
        ...
        foreach ($data['labs'] as $lab) {
            ...
            $option = '<a class="btn btn-xs btn-success invoicebutton" style="margin-right: 10px" target="_blank" href="lab/printLabel?id=' . $lab->id . '">Label Print</a><a class="btn btn-xs btn-warning" href="lab/pdfLabel?id=' . $lab->id . '".><i class="fas fa-file"></i></a>';
            ...
            if ($lab->invoice_id != null) {
                ...
                if ($invoice_details) {
                    $invoice_date_time = date('d-m-Y H:i:s', $invoice_details->date);
                } else {
                    $invoice_date_time = "";
                }
            } else {
            ...
            $info[] = array(
                ...
                $test_name,
                $status,
                $lab->test_status_date != null ? date('d-m-Y H:i:s', $lab->test_status_date) : "",
                $bill_status,
                $lab->done_by,
                $option
                //$options1 . ' ' . $options2 . ' ' . $options3,
    ...
    function getLab()
    {
        ...
        foreach ($data['labs'] as $lab) {
            ...
            $options2 = '<a class="btn btn-xs invoicebutton" title="' . lang('lab') . '" style="color: #fff;" href="lab/invoice?id=' . $lab->id . '"><i class="fa fa-file"></i> ' . lang('') . '</a>';
            $options2 = '<a class="btn btn-xs invoicebutton" title="Download PDF" target="_blank" style="color: #fff; margin-right: 5px" href="lab/viewReport?id=' . $lab->id . '"><i class="fas fa-eye"></i></a>'
                . '<a class="btn btn-xs invoicebutton" title="Download PDF" style="color: #fff; margin-right: 5px" href="lab/testPdf?id=' . $lab->id . '"> PDF</a>'
            ...
            $info[] = array(
                ...
                //$lab->test_status_date != null ? date('d-m-Y H:i:s', $lab->test_status_date) : "",
                $bill_status,
                $lab->updated_on != null ? date('d/m/Y H:i:s', $lab->updated_on) : "",
                $reported_by,
                $options1 . ' ' . $options2 . ' ' . $options3,
            );
        }
    ...
    function getDeliveryReport()
    {
        ...
        foreach ($data['labs'] as $lab) {
            ...
            if ($lab->delivery_status == 'pending' || ($lab->delivery_status == 'delivered' && $lab->receiver_name == null)) {
                $options3 .= ' <a class="btn btn-success btn-xs changeDeliveryStatus" data-id="' . $lab->id . '"><i class="fas fa-check"></i></a>';
            } else {
                $options3 .= ' <a class="btn btn-success btn-xs changeDeliveryStatus" data-id="' . $lab->id . '"><i class="fas fa-check"></i></a>';
            }
            ...
            $info[] = array(
                ...
                $status,
                $lab->delivery_status_date != null ? date('d-m-Y H:i:s', $lab->delivery_status_date) : "",
                $lab->receiver_name,
                $bill_status,
                $options1 . ' ' . $options2 . '' . $options3,
            );
    ...
    public function addLab()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (empty($id)) {
                ...
                echo $this->input->post('submission_type');
