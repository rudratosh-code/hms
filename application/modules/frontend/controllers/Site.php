...
class Site extends MX_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('gridsection_model');
    ...
    public function index() {
        ...
        $this->load->view('site/site/frontend', $data);
    }
    ...
    public function two() {
        ...
        $data['services'] = $this->service_model->getService();
        $data['featureds'] = $this->featured_model->getFeatured();
        $this->load->view('site/site/frontend_backup', $data);
    }
    ...
    public function settings() {
        ...
        $this->load->view('site/site/settings', $data);
    ...
    public function update() {
        ...
        $url_link = $this->input->post('url_hospital');
    ...
    function availableSiteName() {
        $url = $this->input->get('id');
        $get_details_url_exist_or_not = $this->site_model->getSiteSettingsByUrl($url);
        if (empty($get_details_url_exist_or_not)) {
            $avaiable = '1';
        } else {
            if ($this->session->userdata('hospital_id') == $get_details_url_exist_or_not->hospital_id) {
                $avaiable = '1';
            } else {
                $avaiable = '0';
            }
