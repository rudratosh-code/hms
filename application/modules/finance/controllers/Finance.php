...
// Validating Deposited Amount Field
    ...
    function download() {
        ...
        if($data['settings']->invoice_choose=='invoice1'){
            ...
            $html = $this->load->view('invoice_A4', $data, true);
        }else{
            $mpdf = new \Mpdf\Mpdf(['format' => 'A5','autoMarginPadding' => 1,'setAutoTopMargin' => 'true']);
...
$amount[]=array();
        ...
        foreach ($data['payments'] as $payment) {
            ...
            $deposit = ' <a type="button" class="btn btn-xs btn-primary depositButton" title="' . lang('deposit') . '" data-toggle = "modal" data-id="' . $payment->id . '"><i class="fa fa-money"> </i> ' . lang('deposit') . '</a>';
            $doctor_details = $this->doctor_model->getDoctorById($payment->doctor);
